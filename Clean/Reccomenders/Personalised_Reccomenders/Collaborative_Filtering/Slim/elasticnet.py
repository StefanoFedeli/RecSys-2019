...
class SLIMElasticNetRecommender(object):
    ...
    def fit(self, l1_penalty=0.3, l2_penalty=0.6, positive_only=True, topK=20):
        ...
        self.model = ElasticNet(alpha=0.1,
...
# %% [code]
URM = sps.load_npz("../input/datasetrecsys/data_train.npz")
...
with open("output2.csv", "w") as f:
    ...
    for row in validation.itertuples():
        if count % 500 == 0:
            print("Elaborated {} users on {}".format(count, 18000))
        recommendations = np.array2string(recommender.recommend(row.user))
...
# %% [code]
with open("output_all1.csv", "w") as f:
    count = 0
    for row in validation.itertuples():
        if count % 500 == 0:
